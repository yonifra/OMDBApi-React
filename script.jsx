...
var Card = React.createClass({
  ...
  componentDidMount: function() {
    var component = this;
    $.get(
      "https://www.omdbapi.com/?t=" + this.props.login + "&plot=full",
      function(data) {
          ...
          <div className="media">
            <div className="media-left">
              ...
              <div className="media">
                ...
                {this.state.Runtime} <br />
                <span className="glyphicon glyphicon-volume-up" />{" "}
                {this.state.Language}{" "}
              </div>
              <div className="media"> {this.state.Rated} </div>
              <div className="media">
                <a href={imdbUrl} target="_blank">
                  <img
                    alt="IMDb Rating"
                ...
                & nbsp; <b> {this.state.imdbRating} </b>{" "}
              </div>
              <div className="media">
                <img
                  alt="Metacritic Score"
                  src="http://static.metacritic.com/images/icons/mc_fb_og.png"
                  height="20"
                />
                & nbsp;{" "}
                <b>
            ...
            <div className="media-body">
              <h4 className="media-heading">
                {" "}
                <b> {this.state.Title} </b>&nbsp;({this.state.Year})
              </h4>{" "}
              {this.state.Genre} <br />
              <div className="well"> {this.state.Plot} </div>{" "}
              <div className="panel panel-default">
                <div className="panel-heading">
                  <span className="glyphicon glyphicon-facetime-video" /> Cast{" "}
                </div>{" "}
                <div className="panel-body"> {this.state.Actors} </div>{" "}
              </div>{" "}
              <div className="panel panel-default">
                <div className="panel-heading">
                  <span className="glyphicon glyphicon-film" /> Directed by{" "}
                </div>{" "}
                <div className="panel-body"> {this.state.Director} </div>{" "}
              </div>{" "}
              <div className="panel panel-default">
                <div className="panel-heading">
                  <span className="glyphicon glyphicon-pencil" /> Written by{" "}
                </div>{" "}
                <div className="panel-body"> {this.state.Writer} </div>{" "}
              </div>{" "}
            </div>{" "}
          </div>{" "}
        </div>
  ...
  render: function() {
    ...
    return (
      <div className="panel">
        ...
        <div className="panel panel-body"> {cards} </div>{" "}
      </div>
    );
  }
});
